...
プロジェクト経歴の一覧(mypage[実績]タブ)と登録・編集・削除の単独画面(project-form)を提供し、保存・削除時に経験月数を自動再計算する(docs/screens.md project-form・mypage[実績]タブ)。
...
- `app/(authenticated)/projects/new/page.tsx`・`[id]/page.tsx`+`actions.ts`: project-form単独画面
  - 入力項目・バリデーションは docs/screens.md project-form の表のとおり(現場はマスタからサジェスト選択、タイトル必須100文字、期間は開始必須+「現在」チェックで終了入力不可+終了は開始以降(同月可)、概要・業務詳細300文字、役割1つ以上、規模20文字)
  - 担当工程は「調査分析／要件定義／基本設計／詳細設計／製造／テスト／運用」の7項目(複数選択ピル)。この表記ラベルを画面・AI生成プロンプト等の全箇所で統一する
  - 使用スキル: mypageのスキルセクションと同じタグ型UI(習熟度なし。タグ押下でバージョンのみ変更可)。同一スキル(同一バージョン)の重複行はサーバー側でも弾く
  - 削除ボタンは誤操作防止のため左端に配置し、confirm-dialog(文言「このプロジェクト経歴を削除してもよろしいですか？」)で確認
