...
保有スキル(カテゴリ・スキル名・バージョン・習熟度)をREF004[表紙]タブのセクションとして登録・編集できるようにする(docs/screens.md EDT003)。
...
- 閲覧表示: カテゴリ別に「スキル名(バージョン) 記号」のピルを横並び表示し、凡例(◎得意/○経験あり/△基礎知識)をセクション末尾に出す(REF003と共通の表示)
