...
- `app/(authenticated)/master/sites/page.tsx`+`actions.ts`+`components/master/SiteMasterManager.tsx`: master-sites
  ...
  - 主管部署は**部署(unit_level=department)のみ**から単一選択し、「事業部 / 部」形式で表示。`site.organization_unit_id`に保存(サーバー側でも「部署かつ未削除」を再検証)
  ...
  - 同名の論理削除済み現場の復活に対応する。復活時は主管部署を今回の入力値で上書きする(idと作成日時は元の行のまま)

## 受け入れ基準

- 追加・編集(主管部署変更含む)・削除・絞り込みが仕様どおり動く
- 主管部署の選択肢が部署のみに限定される
- 参照中現場の削除がブロックされ、削除済み同名の再登録が復活として動き、主管部署が今回の入力値になる
