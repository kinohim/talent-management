...
- エラー表示: Server Action経由(開発用ログイン)とクエリ`?error=<code>`経由(OAuth)の両方を`lib/auth-errors.ts`で日本語文言に変換して表示する(未登録・退職済み・プロバイダ不一致。文言はdocs/screens.md login)
- ログイン成功後の遷移: 一般社員/管理職は`is_registered=false`なら`/basic-info`(basic-info)へ、それ以外は`/`(home)へ

## 受け入れ基準

- docs/screens.md login のボタン構成・配色・準備中表記・エラー文言を満たす
- 3種のエラー(未登録・退職済み・プロバイダ不一致)がそれぞれ正しい日本語文言で表示される
- 初回ログインのロール別遷移(basic-info直行/home直行)が仕様どおり動く

## 検証方法

1. Playwrightで開発用ログインの成功・未登録エラー・退職者エラーを確認する
2. `is_registered=false`の一般社員でログインし`/basic-info`へ、人事・営業でログインし`/`へ遷移することを確認する
