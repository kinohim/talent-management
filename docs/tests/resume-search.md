...
経歴書一覧(resume-list)の検索フィルタ・ソートのパース処理、経歴書の閲覧表示(resume-detail/mypage)の表示整形、および組織単位に基づく閲覧範囲判定・組織単位管理のロジックをテストする。
...
概要: ソート済みの社員スキル一覧から、連続する同カテゴリを 1 グループにまとめる(経歴書閲覧のカテゴリ別スキル表示用)
前提: モックなし（純粋関数）
...
対象: `lib/resume-view.ts` / テスト: `lib/resume-view.test.ts`
概要: 担当工程フラグのうち true のものだけを日本語ラベルの配列にする(経歴書閲覧のプロジェクト経歴カード表示用)
前提: モックなし（純粋関数）
...
対象: `lib/organization-unit.ts` / テスト: `lib/organization-unit.test.ts`
概要: 一般社員が経歴書詳細(resume-detail)を閲覧できる相手(閲覧範囲)の組織単位 id の集合を算出する(経歴書一覧の「詳細」導線の出し分けに使う。閲覧範囲ルール a/b を 1 回の集合計算に落とし込んだもの)
前提: モックなし（純粋関数）
...
対象: `lib/organization-unit.ts` / テスト: `lib/organization-unit.test.ts`
概要: 経歴書詳細(resume-detail)のアクセス可否。本人・人事・営業・管理職は常に閲覧可、一般社員が他社員を見る場合のみ閲覧範囲判定に従う
前提: モックなし（純粋関数）
...
対象: `lib/organization-unit.ts` / テスト: `lib/organization-unit.test.ts`
概要: 所属組織を「事業部 / 部署 / Gr」の形式で連結する(経歴書閲覧の所属組織表示用)
前提: モックなし（純粋関数）
