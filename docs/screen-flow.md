...
- pdf-preview（PDF出力プレビュー）は経歴書一覧・resume-detail発とmypage発でURLが分かれる
  （/resumes/[employeeId]/pdf-preview と /mypage/pdf-preview）。PDF出力の権限は
  本人・人事・営業／管理職のみ
...
flowchart TD
    ...
    subgraph output["出力"]
        pdfpreview[pdf-preview PDF出力プレビュー]
    end
    ...
    subgraph admin["管理（管理職のみ）"]
        ...
        sitesearch[site-search 現場/社員最寄駅マップ]
    end
    ...
    home -->|"現場/社員最寄駅マップ（管理職）"| sitesearch
    ...
    skillmap -->|"保有者名（閲覧範囲内のみ）"| resumedetail

    %% ---- 出力（プレビューで確認→ダウンロード） ----
    resumelist -->|"PDF（人事・営業／管理職のみ）"| pdfpreview
    resumedetail -->|"PDF出力（人事・営業／管理職のみ）"| pdfpreview
    mypage -->|"PDF出力"| pdfpreview
```
...
- PDF出力は「PDF/PDF出力ボタン→pdf-preview→ダウンロード」の2段階フロー。Excel出力は提供しない
