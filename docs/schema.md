...
|---|---|---|---|
| id | SERIAL | PK | |
| skill_category_name | VARCHAR(100) | UK, NOT NULL | MST001で管理。certification_categoryとは独立。スキルカテゴリ内で一意 |
...
|---|---|---|---|
| id | SERIAL | PK | |
| certification_category_name | VARCHAR(100) | UK, NOT NULL | MST002で管理。skill_categoryとは独立。資格カテゴリ内で一意 |
...
- スキル名・資格名・現場ポジション名・現場名はいずれもシステム全体でユニーク（カテゴリをまたいでも重複不可）。スキルカテゴリ名・資格カテゴリ名もそれぞれのマスタ内でユニーク（DB制約）。組織単位名は同一親の配下内で一意（アプリ側チェック）。
