...
- 事業部（division）はmaster-org-units画面の専用フォームからのみ追加できる。部署・Gr（department/group）は一覧の該当行から「配下に追加」して作成する。
- 削除は次のいずれかの参照が残っている間は不可（「使用中のため削除できません」）: 配下の子組織、所属社員（`employee.organization_unit_id`）、主管部署として参照する現場（`site.organization_unit_id`）。参照カウントはいずれも未削除（`deleted_at IS NULL`）の行のみを対象とする（`lib/organization-unit.ts`）。
...
| is_registered | BOOLEAN | NOT NULL, DEFAULT FALSE | 初期登録完了フラグ。basic-info（初回登録）完了でTRUEに更新。人事・営業はbasic-infoを通らないため初回ログイン成立時に自動でTRUE |
...
| gender | ENUM | NULL可 | male:男性, female:女性, other:その他 |
| experience_months | INT | NULL可 | プロジェクト経歴の登録・更新・削除時に自動計算して保存。**全プロジェクト期間の和集合**（重複期間は1回として数える）の月数（区間は開始月・終了月の両端を含む）。`end_date=NULL`（進行中）は計算時点の年月（JST基準）まで含める。年へは切り捨てず月数のまま保存し、表示は「◯年◯か月」（resume-detail/mypage）、一覧・検索は12で割った年数で扱う |
| career_summary | TEXT | NULL可 | 経歴概要 |
| self_pr | TEXT | NULL可 | 自己PR |
| nearest_station_prefecture | VARCHAR(100) | NULL可 | 3段階プルダウンで選択した都道府県。編集し直したときに選択状態を復元するために保存する（自由記述ではなく`lib/prefectures.ts`の47都道府県のいずれか） |
| nearest_station_line | VARCHAR(100) | NULL可 | 都道府県→路線→駅の3段階セレクトで選択した路線名（自由記述ではなくHeartRails Express APIが返す実在の路線名） |
| nearest_station_name | VARCHAR(100) | NULL可 | 同上で選択した駅名。例：渋谷（HeartRails Express APIの表記に合わせ「駅」サフィックスなし） |
| final_school_name | VARCHAR(100) | NULL可 | 自由記述 |
| final_department_name | VARCHAR(100) | NULL可 | 自由記述（学部・学科名） |
| final_school_type | ENUM | NULL可 | high_school:高校, vocational_school:専門学校, junior_college:短大, university:大学, graduate_school:大学院 |
| graduation_year_month | DATE | NULL可 | 年月のみ意味を持つ（1日固定で保存） |
| graduation_status | ENUM | NULL可 | graduated:卒業, withdrawn:中退 |
...
- **`created_by` / `created_program` / `updated_by` / `updated_program` がNULL可**。監査カラムが設定されるのはaccount-new（管理職による事前登録）実行時のみで、Auth.js自身による更新（ログイン時の`emailVerified`更新等）は監査情報を持たないため。
...
| last_login_at | TIMESTAMP | NULL可 | ログイン成功のたびに更新（account-list「最終ログイン」列）。監査カラムも本人の社員IDと`"login"`で更新する |
...
3. 該当employeeが `employment_status = retired` → 「無効化済み」エラー（開発用ログインでも同じ退職チェックを行う）
...
6. ログイン成立時: `last_login_at` を更新。人事・営業は経歴書を作成しない（basic-infoを通らない）ため、`employee.is_registered` を自動でTRUEにし、`employee.name` が未設定ならSSOの表示名で補完する
7. ログイン後は常にトップ（`/`）へリダイレクトする。一般社員／管理職で `is_registered=false` の場合のbasic-info（初回登録）への誘導は、認証必須ページ共通のガード（`lib/auth-routing.ts` の`resolveDestination`）が毎リクエスト行う（ログイン時に一度だけ判定するのではない）
...
`accounts` に行が作られるのはOAuthログイン（現状は `github`）のみ。開発用ログイン（Credentialsプロバイダ）はAuth.jsの仕様上 `accounts` に紐付けを作らない。

| テーブル | 列名 | 型 | 制約 | 説明 |
|---|---|---|---|---|
| accounts | id | TEXT | PK | cuid |
| accounts | user_id | TEXT | FK→users.id (CASCADE), NOT NULL | |
| accounts | provider | TEXT | NOT NULL | 例: `github` |
...
|---|---|---|---|
| id | SERIAL | PK | |
| skill_category_name | VARCHAR(100) | UK, NOT NULL | master-skillsで管理。certification_categoryとは独立 |
...
| has_version | BOOLEAN | NOT NULL, DEFAULT FALSE | バージョン管理有無。master-skillsでの保存時、バージョンが1件以上あればTRUE、0件ならFALSEに自動設定 |
...
|---|---|---|---|
| id | SERIAL | PK | |
| certification_category_name | VARCHAR(100) | UK, NOT NULL | master-certificationsで管理。skill_categoryとは独立 |
...
|---|---|---|---|
| id | SERIAL | PK | |
| site_name | VARCHAR(100) | UK, NOT NULL | master-sitesで管理。システム全体でユニーク |
| organization_unit_id | INT | FK→organization_unit.id, NULL可 | 主管部署。部署（unit_level=department）のみ選択可・任意。保存時にサーバー側でも「部署かつ未削除」を再検証する |
| nearest_station_prefecture | VARCHAR(100) | NULL可 | 3段階プルダウンで選択した都道府県。employeeテーブルと同じ形（編集し直したときの選択状態復元用） |
| nearest_station_line | VARCHAR(100) | NULL可 | 3段階プルダウンで選択した路線名。employeeテーブルと同じ形（自由記述ではなくHeartRails Express APIが返す実在の路線名） |
| nearest_station_name | VARCHAR(100) | NULL可 | 同上で選択した駅名。例：渋谷（「駅」サフィックスなし）。同上 |
...
| industry | VARCHAR(100) | NULL可 | 自由記述。例：金融派生商品 |
| project_summary | TEXT | NULL可 | |
| start_date | DATE | NOT NULL | |
| end_date | DATE | NULL可 | NULL=現在進行中 |
| total_team_size | VARCHAR(100) | NULL可 | 全体人数。自由記述（"約50名"等の幅表現を許容）。画面上の入力上限は20文字（project-form）。DB側は余裕を持たせている |
| team_size | VARCHAR(100) | NULL可 | 担当チーム人数。自由記述。同上（画面上限20文字） |
...
|---|---|---|---|
| id | SERIAL | PK | |
| project_role_name | VARCHAR(20) | UK, NOT NULL | SE, PG, リーダー 等。master-project-rolesで管理。システム全体でユニーク |
...
| overview | VARCHAR(300) | NULL可 | 業務詳細概要 |
| research_analysis | BOOLEAN | NULL可 | 調査分析 |
| requirements_definition | BOOLEAN | NULL可 | 要件定義 |
| basic_design | BOOLEAN | NULL可 | 基本設計 |
| detailed_design | BOOLEAN | NULL可 | 詳細設計 |
| development | BOOLEAN | NULL可 | 製造 |
| testing | BOOLEAN | NULL可 | テスト |
| operation | BOOLEAN | NULL可 | 運用 |
...
- 論理削除（`deleted_at`）は全テーブル共通。物理削除は行わない（Auth.js管理の3テーブルと、mypage・project-formのスキル／資格明細の洗い替え保存を除く。洗い替え＝deleteMany→createManyの全置換）。社員の退職判定には使わず、`employee.employment_status` を使う。
...
- **マスタ行（skill・skill_version・certification・site・project_role・organization_unit）は、他レコードから参照されている間は削除不可**。削除操作時は参照の有無をチェックし、参照があればエラー「使用中のため削除できません」を表示する（confirm-dialog参照）。参照がなくなれば削除できる。参照チェックの対象は参照元の性質に応じて異なり、論理削除される参照元（project_skill・project_role_link・project・組織関連）は未削除行のみを数え、洗い替え（物理削除）される参照元（employee_skill・employee_certification）は全行を数える。
