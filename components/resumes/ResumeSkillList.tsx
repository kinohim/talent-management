...
export function ResumeSkillList({ groups, hideTitle = false }: ResumeSkillListProps) {
  return (
    <section className="flex flex-col gap-4">
      ...
      {groups.length === 0 ? (
        <p className="text-sm text-foreground/60">登録されているスキルはありません。</p>
      ) : (
              ...
              <ul className="flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <li
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full border border-primary/40 bg-primary/10 px-3 py-1 text-sm text-brand"
                  >
                    <span>
